...
class DataFetcher:
	...
	def __init__(self, evaluation_type, batch_size=32, start_itr=0):
		...
		self.available_datasets = ['abstract_scenes_v1', 'abstract_scenes_v1', 'balanced_binary_abstract_scenes']
	...
	def get_current_dataset(self):
		return self.available_datasets[self.current_dataset_idx]
	...
	def images_to_features(self, images_dict):
		...
		print("WARNING: Using random features since MXNET is not supported !")
	...
	def update_state(self, actual_batch_size):
		...
		if actual_batch_size < self.batch_size and self.itr > self.batch_size:
			self.next_dataset()

	def get_state(self):
		return [self.current_dataset_idx, self.itr, self.batch_size, self.evaluation_type]

	def set_state(self, state_tuple):
		self.current_dataset_idx, self.itr, self.batch_size, self.evaluation_type = state_tuple

	def _get_next_batch(self, batch_size):

		questions_all = get_questions_batch(self.itr, batch_size, self.get_questions_path())
		...
		# Link questions with images and annotations using ids
		annotations, images = self.merge_by_id(questions_all,
			annotations_dict_thread.get_ret_val(), images_dict_thread.get_ret_val())
	...
	def get_next_batch(self):
		...
		eof = self.itr == 0 and self.current_dataset_idx == 0
